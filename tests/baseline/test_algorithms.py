...
class TestBaselineDWT(unittest.TestCase):
    ...
    def test_zero_level(self):
        ...
        self.assertTrue(
            np.allclose(self.arr, baseline_dwt(self.arr, max_iter=5, level=0))
        )
    ...
    def test_approx_rec(self):
        ...
        with self.subTest("1D shape"):
            ...
            rec_arr = _dwt_approx_rec(
                arr, level=2, wavelet="db6", mode="constant", axis=-1
            )
            self.assertSequenceEqual(rec_arr.shape, arr.shape)

        with self.subTest("1D along axis"):
            arr2 = np.random.random(size=(101, 104))
            rec_arr2 = _dwt_approx_rec(
                arr2, level=2, wavelet="sym4", mode="constant", axis=1
            )
    ...
    def test_trivial_case_2d(self):
        ...
        self.assertTrue(
            np.allclose(arr, baseline_dwt(arr, max_iter=10, level=None, axis=(0, 1)))
        )
...
class TestbaselineDT(unittest.TestCase):
    ...
    def test_zero_level(self):
        """ baseline computed at the zero-th level should not affect the array """
        self.assertTrue(
            np.allclose(self.arr, baseline_dt(self.arr, max_iter=5, level=0))
        )
