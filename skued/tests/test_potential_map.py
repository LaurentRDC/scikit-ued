...
class TestPotentialSynthesis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.intensities = [np.abs(structure_factor(self.crystal, *reflection))**2 for reflection in self.reflections]
    ...
    def test_trivial(self):
        ...
        potmap = potential_synthesis(self.reflections, np.zeros_like(self.intensities), self.crystal, plane)
    ...
    def test_positive_intensity(self):
        ...
        with self.assertRaises(ValueError):
            potmap = potential_synthesis(self.reflections, self.intensities, self.crystal, plane)
